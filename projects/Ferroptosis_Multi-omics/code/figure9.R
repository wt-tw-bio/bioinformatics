...
# 获取差异表达基因结果
deg_hc <- topTable(fit2,coef = 1,number = Inf)

deg_hc$threshold=factor(ifelse(deg_hc$adj.P.Val < 0.05 & abs(deg_hc$logFC) >= 1, 
                               ifelse(deg_hc$logFC>= 1 ,'Up','Down'),'Not sig'),levels=c('Up','Down','Not sig'))
deg_hc$logP <- -log10(deg_hc$adj.P.Val)
deg_hc_sig <- subset(deg_hc,threshold %in% c("Up","Down"))
write.csv(deg_hc, file = "plot/bulk/hvkp-ckp/deg.csv", row.names = FALSE)

mycol <- scales::hue_pal()(4)[c(1,4)]
library(ggthemes)
# 火山图
p_hvkp_ckp <- ggplot(data = deg_hc,aes(x = logFC,y = logP,color = threshold))+
...
label <- deg_hc[intersect(rownames(deg_hc_sig), c(FRGs$driver$MGI.symbol, FRGs$suppressor$MGI.symbol, FRGs$marker$MGI.symbol)),]
volcano_plot <- p_hvkp_ckp +
...
ggsave("plot/bulk/hvkp-ckp/vlcano_hvkp-ckp.pdf",height = 6,width = 8, plot = volcano_plot)



library(clusterProfiler)
library(enrichplot)
GSEA_input <- deg_hc$logFC
names(GSEA_input) <- rownames(deg_hc)
...
gene_symbol <- bitr(rownames(deg_hc_sig),fromType = "SYMBOL",toType = "ENTREZID",OrgDb = "org.Mm.eg.db")
...
write.csv(result_gsea, file = "plot/bulk/hvkp-ckp/gsea.csv", row.names = FALSE)
write.csv(GO_res, file = "plot/bulk/hvkp-ckp/GO.csv", row.names = FALSE)
write.csv(Kegg_res, file = "plot/bulk/hvkp-ckp/Kegg.csv", row.names = FALSE)
...
ggsave("plot/bulk/hvkp-ckp/gsea.pdf", width = 7, height = 6, plot = p_gsea)

debugonce(gsea_plot)

# 热图
library(pheatmap)
group_heat <- group[grep("HVKP|CKP", group)]
anno <- data.frame("annotation" = as.factor(group_heat))
rownames(anno) <- colnames(result)[grep("HVKP|CKP", colnames(result))]
...
p <- pheatmap(dat_heat,cluster_col=F,cluster_rows = F, 
              ...
              border_color = NA)
pdf("plot/bulk/hvkp-ckp/heatmap_hvkp-ckp.pdf",height = 4,width = 6)
p
dev.off()
